...
const icons: Record<IconTypes, React.FC<React.SVGProps<SVGSVGElement>>> = {
  ...
  spinner: React.lazy(() => import('/public/icons/spinner-icon.svg'))
};
...
const Icon = (iconParams: IconProps) => {
  const iconType = isValidIconType(iconParams.type) ? iconParams.type : 'cryptoViewerLogo';
  const SvgIcon = icons[iconType];
  const svgSize = iconParams.size ? ICON_SIZES[iconParams.size] : ICON_SIZES.medium;
